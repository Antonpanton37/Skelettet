...
const RunningCalculator = () => {
  ...
  const handleCalculate = async () => {
    ...
    try {
      // NOTE - Loading state can be added here
      // Set as early as possible
      // setLoading true
      const response = await fetch(
        'https://backend-1-s6ox.onrender.com/calculate',
        {
          method: 'POST',
      ...
      // NOTE - Error handling for response
      // if (!response.ok) {
      // 	throw new Error(responseData.error || "Något gick fel");
      // 	setLoading(false)
      // }
      ...
      if (responseData.result < 10) {
        ...
        setAdvice(
          'Det är varmt – se till att dricka ordentligt och undvik att springa mitt på dagen.'
        );
      }
      {
        /*API-KEY*/
      ...
      if (location) {
        // NOTE - Use API key in env, checkout .env
        ...
        // 	setLoading(false)
        // }
        // NOTE - Error handling for weather response
        const weatherData = await weatherResponse.json();

        if (weatherData && weatherData.forecast) {
          const forecastHours = weatherData.forecast.forecastday[0].hour;

          const maxTempHour = forecastHours.reduce(
            (max, hour) => (hour.temp_c > max.temp_c ? hour : max),
            forecastHours[0]
          );
  ...
  return (
    <div id="calculator" className="section">
      ...
      <p className="calculator-subtitle">
        Här kan du beräkna ditt PET-värde och få tillhörande råd.
      </p>
      ...
      <div className="calculator-container">
        <div className="calculator-content">
          <form className="calculator-box" onSubmit={handleSubmit}>
            ...
            />
            {/* NOTE - Add change text to loading if is isLoading */}
          ...
          <div className="calculator-result">
            ...
            {result !== null ? (
              <>
                <p>
                  <strong>PET-temperatur:</strong> {result.toFixed(1)}
                </p>
                {forecast.length > 0 && (
                  <p>
                    <strong>Högsta temp i {location}:</strong>{' '}
                    {forecast[0].temp}°C kl {forecast[0].time}{' '}
                    {forecast[0].icon && (
                      <img
                        src={forecast[0].icon}
                        alt="väder"
